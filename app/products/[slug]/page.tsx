...
export default function ProductDetailPage() {
  ...
  const handleArchiveInquiry = (id: string) => {
    ...
    toast("Inquiry marked as reviewed and archived.");
  };
  if (!product) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center space-y-4">
        ...
        <p className="text-xs text-tx-secondary">The requested product slug "{slug}" was not synced within MongoDB schema state.</p>
        <button onClick={() => router.push("/products")} className="px-6 py-2 bg-brand-green text-white font-semibold text-xs rounded">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              {product.applications?.map((app, idx) => (
                <li key={idx} className="flex items-start space-x-2 text-tx-secondary">
                  <span className="mt-1 h-1.5 w-1.5 rounded-full bg-brand-green flex-shrink-0"></span>
                  <span>{app}</span>
                </li>
              ))}
            </ul>
