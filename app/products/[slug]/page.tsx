...
const page = () => {
  return (
    <div>
      const matchSlug = currentRouteMeta.params?.slug;
          const product = products.find(p => p.slug === matchSlug);

          if (!product) {
            return (
              <div className="mx-auto max-w-7xl px-4 py-16 text-center space-y-4">
                <AlertTriangle className="h-12 w-12 text-red-500 mx-auto" />
                <h2 className="text-bebas text-2xl">Chemical Listing Not Found</h2>
                <p className="text-xs text-tx-secondary">The requested product slug "{matchSlug}" was not synced within MongoDB schema state.</p>
                <button onClick={() => navigate("#/product")} className="px-6 py-2 bg-green-brand text-white font-semibold text-xs rounded">
          ...
          return (
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-8 text-left" id="view-public-detail">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                {/* Left Side: Dynamic Modern Image Carousel */}
                <div className="lg:col-span-5 space-y-4">
                  <ImageCarousel images={product.images} productTitle={product.title} />
                  
                  {/* MOQ highlight */}
                  <div className="rounded-xl border border-green-brand/10 bg-green-alpha/50 p-4 flex items-center space-x-3">
                    <ShieldCheck className="h-5 w-5 text-green-brand flex-shrink-0" />
                    <div className="text-xs">
                      <p className="font-semibold text-green-brand uppercase tracking-wider text-[10px]">REACH Registered Source</p>
                      <p className="text-tx-secondary mt-0.5">Strict quality tracing reports available under lab inquiry submission.</p>
                    </div>
                  </div>
                </div>

                {/* Right Side: Product summary parameters */}
                <div className="lg:col-span-7 space-y-6">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {product.badge && (
                      <span className="rounded bg-green-brand text-white px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                        {product.badge}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold uppercase tracking-wider text-tx-primary flex items-center space-x-1.5 border-b border-border-subtle pb-2">
                      <BookOpen className="h-4 w-4 text-green-brand" />
                    ...
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                      {product.applications?.map((app, idx) => (
                        <li key={idx} className="flex items-start space-x-2 text-tx-secondary">
                          <span className="mt-1 h-1.5 w-1.5 rounded-full bg-green-brand flex-shrink-0"></span>
                          <span>{app}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="pt-4 flex flex-col sm:flex-row gap-4" id="detail-quote-triggers">
                    <button
                      ...
                      className="cursor-pointer flex-1 py-3.5 px-6 rounded-lg bg-green-brand text-white font-bold text-xs uppercase tracking-wider text-center transition-all hover:bg-green-brand/95 shadow-md shadow-green-brand/10"
                    >
                    ...
                    <button
                      onClick={() => setSelectedProductForQuote(product)}
                      className="cursor-pointer py-3.5 px-6 rounded-lg bg-tx-primary hover:bg-tx-secondary text-white font-bold text-xs uppercase tracking-wider text-center transition-all"
                    >
              ...
              {/* Bottom Full Specifications Sheet Matrix table */}
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 font-mono-custom text-tx-secondary">
                      ...
                      <tr className="hover:bg-bg-subtle/50 transition-colors">
                        ...
                        <td className="py-3 px-4 text-green-brand font-semibold">{product.specs.purity || "99.0%+"}</td>
                      </tr>
                      <tr className="hover:bg-bg-subtle/50 transition-colors">
                      ...
                      <tr className="hover:bg-bg-subtle/50 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <span className="inline-flex items-center space-x-1.5 text-tx-primary font-medium">
                            <AlertTriangle className="h-3.5 w-3.5 text-gold-brand" />
